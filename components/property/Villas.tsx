...
const Listings: React.FC = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      ...
      if (error || !data) {
        console.error("Error fetching villa listings:", error?.message);
        setListingsData([]);
        setLoading(false);
        return;
      }
      ...
      <div className="max-w-6xl mx-auto py-20 px-6 2xl:px-0">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 text-center md:text-start">
          <div>
            ...
            <h1 className="text-2xl font-bold">Featured Villas</h1>
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex justify-center items-center py-20">
            ...
            <p className="ml-3 text-gray-500">Loading villas...</p>
          </div>
        ) : listingsData.length === 0 ? (
          <p className="text-center text-gray-500 py-20">No villas found.</p>
        ) : (
          <motion.div
